 ...
 * is ours. `roomTurntableTextures.ts` carries the palette note.
 *
 * ## No lights, on purpose
 *
 * `room3d.ts` says it plainly: the room has no lights, because direct light, bounce and
 * contact shadow are all in the bake. Adding a light rig for this one object would be
 * the wrong kind of correct — the baked meshes could not receive it, so it would light
 * the machine alone, and a PBR material would then also disagree with the room about
 * colour space. Rendering into a target forces linear output, which is what `postfx`
 * expects from the bake; a `MeshStandardMaterial` would write true linear into the same
 * target the baked shader writes sRGB-as-linear into, and the machine would sit in the
 * shot brighter and flatter than everything touching it.
 *
 * So the machine is shaded by hand, in one small raw `ShaderMaterial` (`createSurface`),
 * writing the same space the bake does. It costs no shadow map — which `quality.ts`
 * deliberately does not spend on phones — and it stays consistent in all three places
 * the room renders: the bench, reel frame 05's target, and the About stage.
 */
 ...
 * captured machine in `/lab/room-preview` until the new one's top, front and shadow side
 * sat at the same values as the old one's, which is the only definition of "matches the
 * room" available when the room is a photograph.
 */
const KEY_DIR = new THREE.Vector3(0.38, 0.86, 0.34).normalize();
const FILL_DIR = new THREE.Vector3(-0.62, 0.34, -0.71).normalize();
...
const screen = (hex: string) => new THREE.Color().setStyle(hex, THREE.NoColorSpace);
...
/*
 * One shading model for the whole machine.
 *
 ...
 * turned aluminium platter, a painted plinth and a chrome arm tube read as three
 * different materials under one daylight, which is all this object needs.
 */
...
export function createRoomTurntable(): DeckRig {
  ...
  function createSurface(options: SurfaceOptions = {}) {
    const material = new THREE.ShaderMaterial({
      ...
      uniforms: {
        uColor: { value: screen(options.color ?? "#ffffff") },
        uMap: { value: options.map ? canvasTexture(options.map) : blank },
        ...
        uKeyColor: { value: screen("#5c4e3c") },
        uFillDir: { value: FILL_DIR },
        uFillColor: { value: screen("#191c22") },
        uSkyColor: { value: screen("#332f29") },
        uGroundColor: { value: screen("#141210") },
      },
    });
  ...
  const pilotDark = screen(PILOT_DARK);
  const pilotLit = screen(PILOT_LIT);
